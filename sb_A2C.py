...
def load_agent(
    ...
    if os.path.exists(agent_file_path):
        A2C_agent = A2C.load(agent_file_path, device="cpu")
    else:
        A2C_agent = A2C(
            policy="MlpPolicy",
            env=env,
        ...
        A2C_agent.save(agent_file_path)
    return A2C_agent
...
def main():
    ...
    while not done:
        action, _states = A2C_agent.predict(observation, deterministic=True)
